...
| D24 | *Added 2026-09-08.* A `@bug` scenario that fails is reported as **failing as documented**, never as a contradiction. | `@bug` marks a scenario asserting behaviour the product does not have yet, so its red is the specification working, not a regression. The default run filters these out, which hid the problem: run `-p bugs` or `-p endpoints` and every open defect surfaced as a `conflict` and failed `npm run report:validate`. `statusForCase` now returns `known-defect` for a `@bug` case both ways it can present — filtered out of the default run, or red in a bugs run — and `computedStatusFor` drops those from the rollup exactly as the default run drops them, unless they are all a surface has, in which case the surface reads `known-defect` rather than falsely proven. Both halves were needed and both were found by the gate itself: adding AZ-19 turned `E-analyze-data-graphs` into a `conflict` on a bugs run, and once that was fixed the same surface still read `unproven` on a default run because one filtered case outranked two passing ones. Writing a defect down must never demote the surface it was found on. |
